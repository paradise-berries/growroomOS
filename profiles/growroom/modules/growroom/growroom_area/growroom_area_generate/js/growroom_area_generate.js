(function ($) {
  Drupal.behaviors.growroom_area_generate = {
    attach: function (context, settings) {

      // Initialize a setting for storing the temporary preview layer.
      Drupal.settings.growroom_area_generate_preview_layer = Drupal.settings.growroom_area_generate_preview_layer || {};
  ...
  Drupal.growroom_area_generate = {
    ...
    preview: function (wkt) {
      ...
      if (Drupal.settings.growroom_area_generate_preview_layer) {
        map.map.removeLayer(Drupal.settings.growroom_area_generate_preview_layer);
      }
      ...
      Drupal.settings.growroom_area_generate_preview_layer = new ol.layer.Vector({
      ...
      map.map.addLayer(Drupal.settings.growroom_area_generate_preview_layer);
    }
  };

  // Define a Drupal ajax command for loading the wkt from the hidden input
  // field and previewing it on the map.
  Drupal.ajax.prototype.commands.growroomAreaGeneratePreview = function() {
    var wkt = $('#generated-wkt input[name="wkt"]').val();
    if (wkt) {
      Drupal.growroom_area_generate.preview(wkt);
    }
  }
}(jQuery));
