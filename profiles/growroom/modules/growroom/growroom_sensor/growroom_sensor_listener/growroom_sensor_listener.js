(function ($) {
  Drupal.behaviors.growroom_area_generate = {
    attach: function (context, settings) {
      ...
      for (var i = 0; i < settings.growroom_sensor_listener.graphs.length; i++) {

        // Get the graph name, id, and data.
        var name = settings.growroom_sensor_listener.graphs[i]['name'];
        var id = settings.growroom_sensor_listener.graphs[i]['id'];
        var data = settings.growroom_sensor_listener.graphs[i]['data'];
